...
class PiNet():
    def __init__(self, num_items_A, num_items_B, num_members=4, embedding_size=100, hidden_size=100, num_layers=1, gpu='0'):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('transfer_B'):
                transfer_output_B,transfer_state_B = self.transfer_B(encoder_output_B, self.len_B, self.hidden_size, self.num_layers, self.keep_prob)
            ...
            with tf.name_scope('transfer_A'):
                transfer_output_A,transfer_state_A = self.transfer_A(encoder_output_A, self.len_A, self.hidden_size, self.num_layers, self.keep_prob)
    ...
    def transfer_B(self, encoder_output_B, len_B, hidden_size, num_layers, keep_prob):
        with tf.variable_scope('transfer_B'):
            ...
            transfer_output_B,transfer_state_B = tf.nn.dynamic_rnn(transfer_cell_B, encoder_output_B, sequence_length=len_B, dtype=tf.float32)#transfer_output_B=[batch_size,timestamp_B,hidden_size], transfer_state_B=([batch_size,hidden_size]*num_layers)     
    ...
    def transfer_A(self, encoder_output_A, len_A, hidden_size, num_layers, keep_prob):
        with tf.variable_scope('transfer_A'):
            ...
            transfer_output_A,transfer_state_A = tf.nn.dynamic_rnn(transfer_cell_A, encoder_output_A, sequence_length=len_A, dtype=tf.float32)#transfer_output_A=[batch_size,timestamp_A,hidden_size], transfer_state_A=([batch_size,hidden_size]*num_layers)     
